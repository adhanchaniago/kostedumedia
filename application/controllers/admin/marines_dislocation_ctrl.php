...
class marines_ctrl extends CI_Controller {
    ...
    public function position() {
        ...
        if (!empty($obj) && ($obj['mar_timestamp_date'] != "" || $obj['mar_timestamp_time'] != "")) {
            $filter = $obj;
            $this->data['marines'] = $this->mar_dislocation_history_dao->table_fetch(null,16, $offset, $filter, null, true);
            $count_data_mar = $this->mar_dislocation_history_dao->count_table_fetch(null,$filter);
        } else {
            $filter = null;
            $this->data['marines'] = $this->marines_dao->table_fetch(1000, $offset, $filter, null, null, true, true);
            $count_data_mar = $this->marines_dao->count_table_fetch($filter);
        }
        ...
        $this->data['marine_logistics'] = null;
        $this->data['corps'] = $this->corps_dao->fetch(1000, 0, null, array('corps_type_id' => '3'));
    ...
    private function fetch_input() {
        $data = array(//'martype_id' => $this->input->post('martype_id'),
...
//            'mar_lon' => toGeoDec($this->getValue('mar_dlon'), $this->getValue('mar_mlon'), $this->getValue('mar_slon'), $this->getValue('mar_rlon')),
    ...
    private function fetch_input_position() {
        $data['mar_id'] = $this->input->post('mar_id');
        $data['mar_lat'] = toGeoDec($this->getValue('mar_dlat'), $this->getValue('mar_mlat'), $this->getValue('mar_slat'), $this->getValue('mar_rlat'));
        $data['mar_lon'] = toGeoDec($this->getValue('mar_dlon'), $this->getValue('mar_mlon'), $this->getValue('mar_slon'), $this->getValue('mar_rlon'));
        $data['mar_isrealtime'] = ($this->input->post('mar_isrealtime') == '') ? 'f' : $this->input->post('mar_isrealtime');
        $data['mar_location'] = $this->input->post('mar_location');
        $data['mar_dpp'] = $this->input->post('mar_dpp');
        $data['mar_desc'] = $this->input->post('mar_desc');
        $data['mar_timestamp_location'] = date("Y-m-d") . ' ' . $this->input->post('mar_timestamp_location');
        $data['mar_personel_desc'] = $this->input->post('mar_personel_desc');
        $data['mar_matpur_desc'] = $this->input->post('mar_matpur_desc');
        return $data;
    }
...
//            $obj_id = array('mar_id' => $obj['mar_id']);
            ...
            if (isset($obj['mar_id']) && $this->marines_dao->by_id(array('mar_id' => $obj['mar_id'])) != null) {
                $obj_id = array('mar_id' => $obj['mar_id']);
                $this->marines_dao->update($obj, $obj_id);
                $saving_mar_id = $obj['mar_id'];
            } else {
                $this->marines_dao->insert($obj);
...
//            $this->save_logistic($saving_mar_id);
            $this->session->set_flashdata("info", "Data Marinir berhasil disimpan.");
    ...
    public function save_position() {
        ...
        if (isset($obj['mar_id']) && $obj['mar_id'] != null) {
            $obj_id = array('mar_id' => $obj['mar_id']);
            //saving historical
            $save_historical = $this->save_historical_position($obj);
            $update = $this->marines_dao->update($obj, $obj_id);
            $this->save_logistic($obj['mar_id']);
            if ($update) {
    ...
    public function edit_position($mar_id = null) {
        $this->preload('/position');
        $obj = $this->filter_param();
        ...
        if ($mar_id == null) {
            ...
            $config['base_url'] = base_url() . self::$CURRENT_CONTEXT . '/edit_position/' . $mar_id;
            ...
            $this->data['logistic_item'] = $this->logistic_item_dao->fetch('3');
            $this->data['marine_logistics'] = $this->marine_logistics_dao->table_fetch($mar_id);

            $to_edit = $this->marines_dao->by_id_($mar_id);
            $this->data['obj'] = $to_edit;
            $this->data['corps'] = $this->corps_dao->fetch(1000, 0, null, array('corps_type_id' => '3'));
            $this->data['marines_type'] = $this->marines_type_dao->key_value();
            $this->data['realtime'] = array('TRUE' => 'Ya', 'FALSE' => 'Tidak');
//            print_r($this->data['obj']);exit();
    ...
    public function view_position($mar_id = null) {
        $this->preload('/position');
        if ($mar_id == null) {
            $this->session->set_flashdata("info", "Marinir tidak ter-identifikasi");
            $this->load_view('admin/marines/marines_position');
            ...
            $config['base_url'] = base_url() . self::$CURRENT_CONTEXT . '/view_position/' . $mar_id;
            $config['total_rows'] = $this->marines_dao->count_table_fetch($filter);
            $config['per_page'] = $this->limit;
            $config['uri_segment'] = 5;
            $config['filter_param'] = $_SERVER['QUERY_STRING'];
...
//            print_r($this->data['obj']);exit();
    ...
    public function delete($mar_id = null) {
        ...
        if ($status_del == false) {
            $this->session->set_flashdata("info", "Delete marine failed,logistic deleted!");
        } else {
    ...
    public function get_list($limit = 16, $offset = 0) {
        ...
        $this->data['marines_unit']  = $this->unit_dao->fetch_data(array('unitcat_id'=>'2'), null, 0, 'unit_name'); 
    }
