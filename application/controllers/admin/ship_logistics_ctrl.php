...
class ship_logistics_ctrl extends CI_Controller {

    public $data;
    public $filter;
    public $limit = 16;
    public static $CURRENT_CONTEXT = '/admin/ship_logistics_ctrl';
...
//        $this->form_validation->set_rules('shiplog_value', 'shiplog_value', 'required');
    ...
    public function index($offset = 0) {
        ...
        $this->load_view('admin/ship_logistics/list_ship_logistics', $this->data);
    }

    public function fetch_record($keys) {
        $this->data['ship_logistics'] = $this->ship_logistics_dao->by_id($keys);
    }

    private function fetch_data($limit, $offset) {
        $this->data['ship_logistics'] = $this->ship_logistics_dao->table_fetch($limit, $offset);
    ...
    private function fetch_input() {
        $data = array('ship_id' => $this->input->post('ship_id'),
            'logitem_id' => $this->input->post('logitem_id'),
            'shiplog_value' => $this->input->post('shiplog_value'));
    ...
    public function save() {
        ...
        if ($this->validate() != false) {
            ...
            if ($this->ship_logistics_dao->by_id($obj_id) != null) {
                $this->ship_logistics_dao->update($obj, $obj_id);
            } else {
                $this->ship_logistics_dao->insert($obj);
            }
            ...
            $this->load_view('admin/ship_logistics/ship_logistics_edit', $this->data);
        }
    }
    ...
    public function edit($ship_id = null) {
        if ($ship_id == null
            ...
            $this->load_view('admin/ship_logistics/list_ship_logistics');
        } else {
            $param = $this->get_list($this->limit);
            $obj_id = array('ship_id' => $ship_id);

            $to_edit = $this->ship_logistics_dao->by_id($obj_id);
    ...
    public function view($ship_id = null) {
        ...
        #prepare link for back to view list
        $this->data['link_back'] = anchor(self::$CURRENT_CONTEXT . 'index/', 'Back', array('class' => 'back'));
        $this->load_view('admin/ship_logistics/view_ship_logistics', $this->data);
    }
    ...
    public function get_list($limit = 16, $offset = 0) {
        ...
        $config['total_rows'] = $this->ship_logistics_dao->count_all();
