...
<script>
    $(document).ready(function(){
        ...
        $('.delete-tab').click(function(){
            ...
            var $dialog = $('<div title="Tipe Pesawat"></div>')
            .html('Semua terkait Tipe Pesawat akan ikut dihapus! Hapus tipe pesawat? <div class="clear"></class>').dialog({
                autoOpen: false,
                width: 280,
                show: "fade",
                hide: "fade",
    ...
    function redirect(tail){
        window.location = "<?php echo base_url() ?>admin/aeroplane_type_ctrl" + tail;
    }
</script>
...
<div id="main">
    ...
    <p class="tit-form">Daftar Tipe Pesawat</p>
    <table class="tab-admin" cellpadding="0" cellspacing="0">
        ...
        <tbody>
            <?php
            $count = 1;
            if (!empty($aeroplane_type)) {
                foreach ($aeroplane_type as $row) {
                    ?>
                    <tr class="<?php echo alternator("row-one", "row-two"); ?>">
                        <td><?php echo $count++; ?></td>
                        <td><?php echo $row->aertype_name; ?></td>
                        <td class="action"> 
                            <?php if (is_has_access('aeroplane_type-edit', $permission) || is_has_access('*', $permission)) { ?>
                                <a href="<?php echo base_url(); ?>admin/aeroplane_type_ctrl/edit/<?php echo $row->aertype_id . '#form-pos' ?>" ><div class="tab-edit"></div></a> 
                            <?php } ?>
                            <?php if (is_has_access('aeroplane_type-delete', $permission) || is_has_access('*', $permission)) { ?>
                                <a href="<?php echo base_url(); ?>admin/aeroplane_type_ctrl/delete/<?php echo $row->aertype_id ?>" class="delete-tab"><div class="tab-delete"></div></a>
                            <?php } ?>
                        </td>
    ...
    <br />
    <?php if (is_has_access('aeroplane_type-edit', $permission) || is_has_access('*', $permission)) { ?>
        <p id="form-pos" class="tit-form">Entri Data Tipe Pesawat</p>
        <form action="<?php echo base_url() ?>admin/aeroplane_type_ctrl/save" method="post" id="addAeroplaneType" enctype="multipart/form-data">
            <ul class="form-admin">
                <?php if (!empty($obj)) { ?>
                    <input class="form-admin" type="hidden" name="aertype_id" value="<?php if (!empty($obj)) echo $obj->aertype_id; ?>" />
                <?php } ?>
                <li>
                    <label>Nama * </label>
                    <input class="form-admin" name="aertype_name" type="text" class="text-medium"
                           value="<?php if (!empty($obj)) echo $obj->aertype_name; ?>" >
                    <?php echo form_error('aertype_name'); ?>
