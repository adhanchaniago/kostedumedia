...
<?php } ?>
        ...
        $('.delete-tab').click(function(){
            ...
            var $dialog = $('<div title="Hapus Pesan Keluar"></div>')
            .html('Pesan yang dihapus tidak dapat dikembalikan lagi. Hapus pesan keluar? <div class="clear"></div>').dialog({
                autoOpen: false,
                width: 280,
                show: "fade",
                hide: "fade",
...
function redirect(tail) {
        window.location = "<?php echo base_url() ?>admin/message_ctrl/outbox" + tail;
    }
function create_url(){
...
<div id="main">
    ...
    <p class="tit-form">Daftar Pesan Keluar<a href="#" id="filtering-form">Table Filter <img src="<?php echo base_url() ?>assets/html/img/arrow-down-black.png" /></a></p>
...
?>
            <tr class="<?php echo alternator("row-one", "row-two"); ?>">
                ...
                <td><?php echo $row->id_to; ?></td>
                <!--<td><?php echo $row->msg; ?></td>-->
                <td><?php echo $row->state_description; ?></td>
                <td class="action">
                    <a href="<?php echo base_url(); ?>admin/message_ctrl/view_outbox/<?php echo $row->msg_id . '?' . http_build_query($_GET).'#form-pos' ?>" ><div class="tab-view"></div></a>
                    <a href="<?php echo base_url(); ?>admin/message_ctrl/delete_outbox/<?php echo $row->msg_id ?>" class="delete-tab"><div class="tab-delete"></div></a>  
                </td>
            </tr>
<?php
        }
           ...
           <!-- <li>
            ...
            <li>
                ...
                <?php foreach ($ships as $row) { 
                    if ((!empty($obj)) && $obj->id_to == $row->ship_id) { ?>
